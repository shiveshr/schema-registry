...
public class EncodingCache {
    ...
    private EncodingCache(String scope, String groupId, SchemaRegistryClient schemaRegistryClient) {
        encodingCache = CacheBuilder.newBuilder().build(new CacheLoader<EncodingId, EncodingInfo>() {
            ...
            public EncodingInfo load(EncodingId key) {
                return schemaRegistryClient.getEncodingInfo(scope, groupId, key);
            }
        });
    }
    ...
    @Synchronized
    public static EncodingCache getEncodingCacheForGroup(String scope, String groupId, SchemaRegistryClient schemaRegistryClient) {
        Key key = new Key(schemaRegistryClient, scope, groupId);
        if (GROUP_CACHE_MAP.containsKey(key)) {
            return GROUP_CACHE_MAP.get(key);
        } else {
            EncodingCache value = new EncodingCache(scope, groupId, schemaRegistryClient);
