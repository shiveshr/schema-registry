...
public interface SchemaValidationRuleRecord {
    ...
    SchemaValidationRule getSchemaValidationRule();
    
    class Serializer extends VersionedSerializer.MultiType<SchemaValidationRuleRecord> {

        @Override
        protected void declareSerializers(Builder builder) {
            // Unused values (Do not repurpose!):
            // - 0: Unsupported Serializer.
            builder.serializer(CompatibilityRecord.class, 1, new CompatibilityRecord.Serializer());
        }
        ...
        public byte[] toBytes(SchemaValidationRuleRecord value) {
         ...
         */
        @SneakyThrows(IOException.class)
        public SchemaValidationRuleRecord fromBytes(byte[] buffer) {
